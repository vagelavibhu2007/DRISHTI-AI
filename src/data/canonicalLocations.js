...
/**
 * Resolves the most precise geographic coordinates for a given project.
 * 
 * @param {Object} project - The project object
 * @returns {Object} { lat: number, lng: number, precision: string, locationLabel: string, locationType: string }
 */
export function resolveProjectLocation(project) {
  if (!project) {
    return {
      lat: 20.5937,
      lng: 78.9629,
      precision: 'STATE_JURISDICTION',
      locationLabel: 'India Baseline',
      locationType: 'national'
    };
  }

  // 1. Check if explicit lat and lng exist and are valid numbers
  ...
  if (rawLat !== null && rawLng !== null && !isNaN(rawLat) && !isNaN(rawLng) && rawLat !== 0 && rawLng !== 0) {
    return {
      ...
      locationLabel: 'Exact Site Coordinates',
      locationType: 'exact_site'
    };
  }

  // 2. Check district field if present
  ...
  // 3. Match project name keywords against canonical locations
  ...
  for (const [stName, coords] of Object.entries(STATE_CENTROIDS)) {
    if (stName.toLowerCase() === firstState.toLowerCase() || firstState.toLowerCase().includes(stName.toLowerCase())) {
      return {
        ...
        lng: coords[1],
        precision: 'STATE_JURISDICTION',
        locationLabel: stName + ' Jurisdiction',
  ...
  return {
    ...
    lng: 78.9629,
    precision: 'STATE_JURISDICTION',
    locationLabel: (rawState || 'National') + ' Regional',
    locationType: 'state_centroid'
  };
}
